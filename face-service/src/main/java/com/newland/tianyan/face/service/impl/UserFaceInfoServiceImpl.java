...
public class UserFaceInfoServiceImpl implements UserFaceInfoService {
    ...
    @Override
    public FaceInfo create(BackendFacesetFaceAddRequest receive) {
        ...
        for (String groupId : groupIdSet) {
            ...
            if (sourceUser == null) {
                ...
                if (faceCacheHelper.add(insertFace) < 0) {
                    ...
                    throw FaceServiceErrorEnum.CACHE_INSERT_ERROR.toException("[人脸新增]");
                }
                if (faceMapper.insertSelective(insertFace) <= 0) {
                    ...
                    throw FaceServiceErrorEnum.DB_INSERT_ERROR.toException("[人脸新增]");
                }
            ...
            //下面只实现了当action_type取值为"append"时的执行逻辑，而当action_type取值为"replace"时的执行逻辑这里暂不实现。
                ...
                if ("append".equals(receive.getActionType())) {
                    ...
                    if ((!CollectionUtils.isEmpty(faceIdList)) && faceCacheHelper.deleteBatch(query.getAppId(), faceIdList) < 0) {
                        ...
                        throw FaceServiceErrorEnum.CACHE_DELETE_ERROR.toException("[人脸新增]");
                    }
                    int deleteCount;
                    try {
                        ...
                        e.printStackTrace();
                        throw FaceServiceErrorEnum.DB_DELETE_ERROR.toException("[人脸新增]");
                    }
                    //添加该用户新的人脸（只有一张）
                    if (faceCacheHelper.add(insertFace) < 0) {
                        log.info("[人脸新增向量失败],参数{}", "AppId:" + insertFace.getAppId() + "GroupId" + insertFace.getGroupId() + "userId" + insertFace.getUserId());
    ...
    private void uploadImage(FaceInfo face, String image) {
        //提交至指定服务器路径
        String imagePath = imageStoreFeignService.uploadImageV2(image);
        face.setImagePath(imagePath);
    }

    private void handleFeatures(FaceInfo face, String image) {
    ...
    @Override
    public List<FaceInfo> getList(BackendFacesetUserFaceGetListRequest receive) {
        FaceInfo query = new FaceInfo();
        BeanUtils.copyProperties(receive, query);
        ...
        PageInfo<FaceInfo> facePageInfo = PageHelper.offsetPage(query.getStartIndex(), query.getLength())
                .setOrderBy("create_time desc")
                .doSelectPageInfo(
                        () -> faceMapper.select(query));

        List<FaceInfo> faces = facePageInfo.getList();
        if (CollectionUtils.isEmpty(faces)) {
            return faces;
        }
        for (FaceInfo face : faces) {
            if (face.getImagePath() != null) {
                face.setImage(imageStoreFeignService.downloadImage(face.getImagePath()));
            }
    ...
    @Override
    public void delete(BackendFacesetFaceDeleteRequest receive) {
        FaceInfo query = new FaceInfo();
        BeanUtils.copyProperties(receive, query);
        ...
        FaceInfo face = faceMapper.selectOne(query);
        if (face == null) {
            return;
        }
        ...
        if (faceCacheHelper.delete(query.getAppId(), face.getId()) < 0) {
            ...
            throw FaceServiceErrorEnum.CACHE_DELETE_ERROR.toException("[人脸删除]", "faceId:" + face.getId());
        }
        //物理删除人脸
        try {
            ...
            e.printStackTrace();
            throw FaceServiceErrorEnum.DB_DELETE_ERROR.toException("[人脸删除]");
        }
